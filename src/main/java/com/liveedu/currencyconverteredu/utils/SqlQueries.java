...
//import com.epam.beamery.services.model.IntegrationIdToFemidaId;
//import com.epam.beamery.services.model.TransactionLogRecord;
...
public interface SqlQueries {
    ...
    String GET_NEXT_SERVICE_FEMIDA_ID = "SELECT \"FBIntegration\".getNextServiceFemidaId()";
    String GET_CERTAIN_AMOUNT_OF_RECORDS_WITH_STATE_AND_OBJECT_TYPE
    ...
    String GET_CERTAIN_AMOUNT_OF_SENT_PINNED_NOTES
            = "SELECT DISTINCT ON(integration_id) transaction_id,transactions.integration_id,femida_id,beamery_id," +
            "state,object_type,transaction_type,updated_datetime,effective_date_time,dto,beamery_hash_id " +
            "FROM \"FBIntegration\".transactions " +
            "JOIN (SELECT integration_id,femida_id,beamery_id,beamery_hash_id,state,object_type,updated_datetime " +
            ...
            "ON os.integration_id = transactions.integration_id " +
            "ORDER BY integration_id, effective_date_time DESC, transaction_id DESC";
    String SET_BEAMERY_ID_STATE_UPDATED_DATE_TIME_BEAMERY_HASH_ID_FOR_FEMIDA_ID_AND_OBJECT_TYPE
            = "UPDATE \"FBIntegration\".object_states SET beamery_id = ?, state = ?, updated_datetime = ?, beamery_hash_id = ? " +
            "WHERE femida_id = ? and object_type = ?";
    String SET_BEAMERY_ID_STATE_UPDATED_DATE_TIME_FOR_FEMIDA_ID_AND_OBJECT_TYPE
            = "UPDATE \"FBIntegration\".object_states SET beamery_id = ?, state = ?, updated_datetime = ? " +
            "WHERE femida_id = ? and object_type = ?";
    String SET_STATE_FOR_FEMIDA_ID_AND_OBJECT_TYPE
            = "UPDATE \"FBIntegration\".object_states SET state = ? WHERE femida_id = ? AND object_type = ?";
    String SET_STATE_AND_UPDATED_DATETIME_FOR_FEMIDA_ID_AND_OBJECT_TYPE
    ...
    String GET_BEAMERY_ID_BY_FEMIDA_ID_AND_OBJECT_TYPE
    ...
    String SAVE_TRANSACTION_RECORD
    ...
    String SET_UPDATED_DATETIME_FOR_FEMIDA_ID_AND_OBJECT_TYPE
            = "UPDATE \"FBIntegration\".object_states " +
            "SET updated_datetime = ?, retry_count = 0, next_retry_datetime = now() WHERE femida_id = ? AND object_type = ?";
    String GET_LATEST_TRANSACTION_LOG_RECORD_BY_FEMIDA_ID_AND_OBJECT_TYPE
    ...
    String SELECT_ROW_COUNT_FROM_CANDIDATE_TO_NOTES_MAPPING_BY_CANDIDATE_ID_AND_TYPE
    ...
    String INSERT_INTO_CANDIDATE_TO_NOTES_MAPPING
    ...
    String GET_NOTE_FEMIDA_ID_BY_CANDIDATE_ID_AND_TYPE_BY_CANDIDATE_FEMIDA_ID
    ...
    String UPDATE_RETRY_COUNT_AND_DATETIME_BY_INTEGRATION_ID
            = "UPDATE \"FBIntegration\".object_states " +
    ...
    String GET_RETRY_RECORDS_WITH_OBJECT_TYPE_AND_LIMIT
            = "SELECT DISTINCT ON (integration_id) transaction_id,transactions.integration_id,femida_id,beamery_id," +
            "state,object_type,transaction_type, updated_datetime,effective_date_time,dto,beamery_hash_id " +
            "FROM \"FBIntegration\".transactions " +
    ...
    RowMapper<String> STRING_ROW_MAPPER = (rs, rowNum) -> rs.getString(1);
    RowMapper<Long> LONG_ROW_MAPPER = (rs, rowNum) -> rs.getLong(1);
    RowMapper<Long> COUNT_ROW_MAPPER = (rs, rowNum) -> rs.getLong("count");
    RowMapper<String> INTEGRATION_ID_STRING_ROW_MAPPER = (rs, rowNum) -> rs.getString("integration_id");
//    BeanPropertyRowMapper<TransactionLogRecord> TRANSACTION_LOG_RECORD_BEAN_PROPERTY_ROW_MAPPER
//            = new BeanPropertyRowMapper<>(TransactionLogRecord.class);
//    BeanPropertyRowMapper<IntegrationIdToFemidaId> INTEGRATION_ID_TO_FEMIDA_ID_BEAN_PROPERTY_ROW_MAPPER
//            = new BeanPropertyRowMapper<>(IntegrationIdToFemidaId.class);
}
